...
def build_url(config: Dict) -> str:
    ...
    return URL_FORMAT.format(
        model_endpoint=config["model_endpoint"], model_id=config["model_id"]
    )


def build_payload(config: Dict, user_input: str) -> Dict:
    """
    ...
    """
    return {"inputs": user_input, "parameters": config["parameters"]}
...
def call_api(session: Session, config: Dict, auth_token: str, user_input: str) -> str:
    """
